...
def save_baseline_results(out_dir, dataset, args, embedding, y_true, y_pred, metrics):
    ...
    print(f"Saved scLEGA run outputs to: {run_dir}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--knn', type=int, default=False,
                        ...
                        neighbors more distant than the n_neighbors nearest nei000ghbor. default:False''')
    ...
    parser.add_argument('--loss_rate', type=float, default=2.0,
                        help='modify ZINB loss to make the model concern about low-expressed genes')
    parser.add_argument('--tailor_rate', type=float, default=0.9,
                        help='modify ZINB loss to make the model concern about low-expressed genes')
    ...
    if args.save_umap is True:
        umap_save_path = ['./umap_figure/%s_pred_label.png' % (args.name),
                          './umap_figure/%s_true_label.png' % (args.name)]
    else:
    ...
    if adata.shape[0] < args.max_num_cell:
        ...
        print(args)
        init_model = scLEGA(256, 64, 64, 256, n_input=args.n_input, n_z=args.n_z, heads=args.n_heads,
                                 device=device)
    ...
    # down-sampling input
        ...
        copy_model = scLEGA(256, 64, 64, 256, n_input=args.n_input, n_z=args.n_z, heads=args.n_heads,
