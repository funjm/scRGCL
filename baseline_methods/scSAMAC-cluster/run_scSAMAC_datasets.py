#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Batch run scSAMAC on a fixed set of datasets.

Usage examples:
  python run_scSAMAC_datasets.py --datasets all
  python run_scSAMAC_datasets.py --datasets 0,9,11
  python run_scSAMAC_datasets.py --datasets Quake_10x_Bladder,Pollen
"""
...
def main():
    parser = argparse.ArgumentParser(description='Batch run scSAMAC on multiple datasets')
    ...
    parser.add_argument('--script', default='/disk/fanjunming/home/workspace/bioinfo/scRGCL/baseline_methods/scSAMAC-cluster/run_scSAMAC.py', help='Path to the scSAMAC entry script')
    parser.add_argument('--out-dir', default='/disk/fanjunming/home/workspace/bioinfo/scRGCL/baseline/results/scSAMAC-cluster', help='Directory for per-run metrics, predictions, embeddings, and summary files')
    ...
    print(f'Running scSAMAC for {len(datasets)} dataset(s): {datasets}')
    failed_datasets = []
    for ds in datasets:
        cmd = [args.python, str(script_path), '--data_file', ds, '--out-dir', args.out_dir]
        if args.extra_args:
